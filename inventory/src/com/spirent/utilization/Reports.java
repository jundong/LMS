		...
		try {
			...
			if (conn != null) {
				...
				if (isResourcesReport != null){
				    ...
				    response.sendRedirect("/inventory/reports.jsp?topList=" + topList + "&lastList=" + lastList + "&isResourcesReport=true&isEmpty=" + isEmpty);
				} else if(isUserReport != null) {
					Map<String, Integer> mapLogin = new TreeMap<String, Integer>();
					String fullName = "";
					sqlStr = "SELECT domainname, logintimes FROM users";
					rs = stmt.executeQuery(sqlStr);
					...
					response.sendRedirect("/inventory/reports.jsp?topList=" + topList + "&lastList=" + lastList + "&isUserReport=true&isEmpty=" + isEmpty);
				}
			}
		} catch (Exception e) {
