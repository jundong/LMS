			...
			try {
				...
				if (conn != null) {
	                ...
	                if (isSetting != null) {
						...
						SQLstmt = "update users set receive='" + receivemail + "', recresreport='" + recresreport + "', monitorreceive='" + monitorreport + "', recloginreport='" + recloginreport+ "', loanerreceive='" + loanerreceive + "' where username='"
								+ request.getSession().getAttribute("username") + "'";
						    stmt.execute(SQLstmt);
						    
						response.sendRedirect("/inventory/preference.jsp");
						
				    }  else if (isUpdateSite != null) {
				    	String selectsite = request.getParameter("site");
				    	
						//Delete record in local DB server 
	                	SQLstmt = "update users set site ='" + selectsite + "'  where username='" + request.getSession().getAttribute("username") + "'";
						stmt.execute(SQLstmt);
	
						request.getSession().setAttribute("loginsite", selectsite);
						response.sendRedirect("/inventory/index.jsp?Animation=true");
				    }
				}
			} catch (Exception e) {
