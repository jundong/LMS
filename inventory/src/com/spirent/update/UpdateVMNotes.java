		...
		try {
			...
			if (conn != null) {
				...
				if (isVM != null){
					...
					response.sendRedirect("/inventory/tree/vmclient.jsp?ClientIp="+VMClientIp.trim());
                } else if (isPC != null){
					...
					response.sendRedirect("/inventory/tree/dut.jsp?Site="+request.getSession().getAttribute("loginsite"));
					...
					response.sendRedirect("/inventory/tree/dutIntf.jsp?Ip="+DUTIntfIP.trim());
					...
					response.sendRedirect("/inventory/tree/slot.jsp?Ip="+ChSlotIP.trim());
                } else if (isPort != null){
					...
					response.sendRedirect("/inventory/tree/port.jsp?Ip="+ChPortIP.trim());
                } else if (isAV != null){
					...
					response.sendRedirect("/inventory/tree/avport.jsp?Ip="+AVPortIp.trim());
                }
			}
		} catch (Exception e) {
