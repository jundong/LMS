...
puts "\nLoad the SPI_AV package..."
    ...
    set chassisIpList [::mysql::sel $::dbConn "SELECT ipaddress FROM avl_appliances" -list]
    
    #set chassisIPList "10.47.97.115"
    #Process one connected physicalchassis at a time
    foreach IpAddress $chassisIpList {
        ...
        #if {[catch {SPI_AV::Appliance::ReleaseAppUnits $IpAddress} error]} {
        #    puts "Error disconnecting from $IpAddress. $error"
        #} else {
        #    puts "Disconnected from $IpAddress."
        #}
    }

    mysql::CloseMySqlDB $::dbConn

    #wait for 1 hours to update the inventory database system again     
