		...
		<div class="content">
			...
			<table BORDER="0" CELLPADDING="0" CELLSPACING="0" align="left"
				...
				<tr bgcolor=<%=color%>>
					...
					<td>
						<A
							HREF="/inventory/update/connection.jsp?Ip=<%=rs.getString("Hostname")%>&Slot=<%=rs.getString("SlotIndex")%>&PortIndex=<%=rs.getString("PortIndex")%>&RadioConnection=<%=rs.getString("ConnectionType")%>&PeerHost=<%=rs.getString("PeerHost")%>&PeerModule=<%=rs.getString("PeerModule")%>&PeerPort=<%=rs.getString("PeerPort")%>&Comments=<%=rs.getString("Comments")%>"
					...
					<td>
						<a HREF="/inventory/update/notes.jsp?ChPortIP=<%=rs.getString("Hostname")%>&ChSlotPortIndex=<%=rs.getString("SlotIndex")%>&ChPortIndex=<%=rs.getString("PortIndex")%>&ChPortNotes=<%=rs.getString("Notes")%>" target="_parent"><%=rs.getString("Notes")%>&nbsp;&nbsp;</a>
					</td>						
					<td>
