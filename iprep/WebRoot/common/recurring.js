...
scheduler._del_rec_marker = function(id){
	var ev = this._rec_markers[id];
	delete this._rec_markers_pull[ev.event_pid][ev._pid_time];
	delete this._rec_markers[id];	
};
scheduler._rec_temp = [];
...
scheduler.attachEvent("onEventChanged",function(id){
	...
	if (id.toString().indexOf("#")!=-1) {
		...
		var sub = this._get_rec_markers(id);
		for (var i=0; i < sub.length; i++) {
			this.deleteEvent(sub[i],true);		
			this._del_rec_marker(sub[i]);
		};
	}
	return true;
});
