		...
		<div class="content">
			...
			<table BORDER="0" CELLPADDING="1" CELLSPACING="0" align="left"
				...
				<tr bgcolor=<%=color%>>
					...
					<td>
						<a
							HREF="/iprep/tree/pc.jsp?Site=<%=rs.getString("Site")%>" target="_parent"><%=rs.getString("Site")%>&nbsp;&nbsp;</a>
					</td>
					<td>
						<a HREF="/iprep/update/location.jsp?ip=<%=rs.getString("DNSHostName")%>&location=<%=rs.getString("Location")%>&type=PC"><%=rs.getString("Location")%>&nbsp;&nbsp;</a>
					</td>
					<td>
						<a HREF="/iprep/update.jsp?PCIp=<%=rs.getString("DNSHostName")%>" target="_parent"><%=rs.getString("Dept")%>&nbsp;&nbsp;</a>
					</td>
					<td>
...
<%
					...
					<td>
						<a HREF="/iprep/update/notes.jsp?DNSHostName=<%=rs.getString("DNSHostName")%>&DNSHostNotes=<%=rs.getString("Notes")%>" target="_parent"><%=rs.getString("Notes")%>&nbsp;&nbsp;</a>
					</td>																
					<td>
				...
				<%
						...
						System.out
								.println("Error occourred in treepc.jsp: "
										+ e.getMessage());
					} finally {
			        	try {
			        		if(rs != null){
			    			...
			    			System.out
							.println("Close DB error occourred in treepc.jsp: "
									+ e.getMessage());
			        	}
			        }
				%>
