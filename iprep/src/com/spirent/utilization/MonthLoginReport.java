...
public class MonthLoginReport extends TimerTask {
	private ServletContext context = null;
	
	public MonthLoginReport(ServletContext context){
    ...
    public void run() {	   
		...
		try {
			...
			if (conn != null) {
				...
				String sqlStr = "SELECT domainname, monloginreport FROM iprep_users";
				rs = stmt.executeQuery(sqlStr);
				
				while (rs.next()){
					fullName = rs.getString("domainname");
					mapLogin.put(fullName, rs.getInt("monloginreport"));
		     	}
				...
				Collections.sort(listLogin, new Comparator<Map.Entry<String, Integer>>() { 
					...
					sqlStr = "SELECT domainname, mail, recloginreport FROM iprep_users WHERE recloginreport='1'";
					rs = stmt.executeQuery(sqlStr);
					while (rs.next()){
						if (!rs.getString("mail").equals("")){
							...
							String subject = "Spirent Lab Management System Monthly Login Report";
			...
			System.out.println("Error occourred in MonthLoginReport.java: "
							+ e.getMessage());
		} finally {
        	try {
        		if(rs != null){
    			...
    			System.out
				.println("Close DB error occourred in MonthLoginReport.java: "
						+ e.getMessage());
        	}
        }  
    }
    ...
    public void resetMonthly() {	   
		...
		try {
			...
			if (conn != null) {
				...
				String sqlStr = "SELECT username FROM iprep_users";
				rs = stmt.executeQuery(sqlStr);	
				...
				for (int i=0; i<loginNamesList.size(); i++) {
					String sqlUpdStr = "UPDATE iprep_users SET monloginreport='0' WHERE username='" + loginNamesList.get(i) + "'";
					stmt.execute(sqlUpdStr);	
				}
			}
		} catch (Exception e) {
			System.out.println("Error occourred in MonthLoginReport.java->resetMonthly: "
							+ e.getMessage());
		} finally {
        	try {
        		if(rs != null){
    			...
    			System.out
				.println("Close DB error occourred in MonthLoginReport.java->resetMonthly: "
						+ e.getMessage());
        	}
        }
    }
